...
// Each call runs a paid TTS synthesis pass — keep the per-IP ceiling modest.
const RATE_LIMIT_MAX = 15;

export async function POST(req: NextRequest) {
  const { limited, retryAfterSeconds } = checkRateLimit('voice-speak', getClientIp(req), RATE_LIMIT_MAX);
  if (limited) {
